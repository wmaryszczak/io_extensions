...
namespace WMA.IOExtensions.Test.Json
{
    public class MultiContentJsonReaderBenchmark
    {
        ...
        {
            await foreach (
                var hotel in System.Text.Json.JsonSerializer.DeserializeAsyncEnumerable<HotelModel>(
                    this.validContentStream
                )
            )
            {
                if (hotel != null)
        ...
        {
            await foreach (
                var hotel in MultiContentJsonReader.DeserializeAsyncEnumerable<HotelModel>(
                    stream,
                    options
                )
            )
