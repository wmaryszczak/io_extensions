...
namespace WMA.IOExtensions.Json
{
  public class MultiContentJsonReader
  {
      ...
      [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
      ...
      try
      {
        while (!cancellationToken.IsCancellationRequested)
        {
          ...
          {
            var obj = ProcessObject<TValue>(payload, options?.JsonSerializerOptions);
            yield return obj;
          }
      ...
      CancellationToken cancellationToken = default)
    {
      ...
      var _ = await TryDeserializeAsync<TValue>(stream, retval, options, cancellationToken);
      return retval;
    }
      ...
      out ReadOnlySequence<byte> obj)
    {
      ...
      else
      {
        ...
        var offset = buffer.Slice(buffer.GetPosition(0, position.Value), 1).FirstSpan[0] == '\n' ? 1 : 0;
    ...
    private static SequencePosition? GetPosition(ReadOnlySequence<byte> buffer, MultiContentJsonReaderOptions? options)
    {
    ...
    private static TValue? ProcessObject<TValue>(ReadOnlySequence<byte> payload, JsonSerializerOptions? options)
    {
